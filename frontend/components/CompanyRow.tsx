...
export default function CompanyRow({ item, onRemove }: Props) {
  ...
  return (
    <div className={`group flex items-center gap-4 bg-slate-900 border rounded-xl px-4 py-3.5 hover:border-slate-700 transition-all ${
      c.risk_score === 'red'
        ? 'border-red-500/30'
        : c.risk_score === 'yellow'
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {item.alias && (
            <span className="text-slate-500 text-xs truncate hidden sm:block">({c.name})</span>
          )}
        </div>
